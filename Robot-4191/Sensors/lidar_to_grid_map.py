...
def calc_grid_map_config(ox, oy, xy_resolution, map_size):
    """
    ...
    max_x = round(max(ox))
    max_y = round(max(oy))
    xw = int(round((map_size[1] - map_size[0]) / xy_resolution))
    yw = int(round((map_size[1] - map_size[0]) / xy_resolution))
...
def init_flood_fill(center_point, obstacle_points, xy_points, min_coord,
    ...
    for (x, y) in zip(ox, oy):
        ...
        if ix >= occupancy_map.shape[0]:
            ix = occupancy_map.shape[0] - 1
        if iy >= occupancy_map.shape[1]:
            iy = occupancy_map.shape[1] - 1
        print((x - min_x), xy_resolution, ix)
        print((y - min_y), xy_resolution, iy)
        print((prev_ix, prev_iy), (ix, iy))
...
def generate_ray_casting_grid_map(ox, oy, xy_resolution, breshen=True, map_size = [-10,10]):
    """
    ...
    if breshen:
        ...
        for (x, y) in zip(ox, oy):
            # x coordinate of the the occupied area
            ix = int(round((x - map_size[0]) / xy_resolution))
            # y coordinate of the the occupied area
            iy = int(round((y - map_size[0]) / xy_resolution))
            if ix >= occupancy_map.shape[0]:
                ix = occupancy_map.shape[0] - 1
            if iy >= occupancy_map.shape[1]:
                iy = occupancy_map.shape[1] - 1
            ...
            for laser_beam in laser_beams:
                #print(laser_beam,occupancy_map.shape )
                occupancy_map[laser_beam[0]][laser_beam[1]] = 0.0  # free area 0.0
                if (laser_beam[0],laser_beam[1]) not in clearPoints.keys():
                    clearPoints[(laser_beam[0],laser_beam[1])] = 0
                #clearPoints.append(laser_beam)
            if ix >= occupancy_map.shape[0]-1:
                ix = occupancy_map.shape[0] - 2
            if iy >= occupancy_map.shape[1]-1:
                iy = occupancy_map.shape[1] - 2
    ...
    # occupancy grid computed with with flood fill
        ...
        for (x, y) in zip(ox, oy):
            ...
            if iy >= occupancy_map.shape[1]-1:
                iy = occupancy_map.shape[1] - 2
            occupancy_map[ix][iy] = 1.5 
            occupancy_map[ix][iy] = 1.5  # occupied area 1.0
            occupancy_map[ix + 1][iy] = 1.5  # extend the occupied area
            occupancy_map[ix][iy + 1] = 1.5  # extend the occupied area
            occupancy_map[ix + 1][iy + 1] = 1.5  # extend the occupied area

    return occupancy_map, min_x, max_x, min_y, max_y, xy_resolution, center_x, clearPoints
